...
def batch_parser(codes, area_type=1, media_path="", with_log=False, catalog_path="", coord_out="EPSG:3857",
                 file_name="example", output=os.path.join("output"), repeat=5, areas=None):
    if areas is None:
        areas = []
    ...
    if len(with_error) and repeat:
        ...
        batch_parser(with_error, area_type=area_type, media_path=media_path, with_log=with_log, catalog_path=catalog_path,
                     coord_out=coord_out, file_name=file_name, output=output, repeat=repeat - 1, areas=areas)
    else:
